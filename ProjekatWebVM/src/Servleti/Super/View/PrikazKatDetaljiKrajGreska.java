...
/**
 * Servlet implementation class PrikazKatDetaljiKrajGreska
 */
@WebServlet("/PrikazKatDetaljiKrajGreska")
public class PrikazKatDetaljiKrajGreska extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrikazKatDetaljiKrajGreska() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!k.korisnik.getUloga().equals("super")){
			...
			response.sendRedirect(k.putanja + "Login.jsp");
				...
				out.println("		<p>RAM: </p><input type=\"text\" value=\""+k.kategorije.get(request.getParameter("ime")).getRam()+"\"/ >");
				if(!ramBul)
					out.println("<p>RAM nije validno</p>");
				out.println("		<p>GPU: </p><input type=\"text\" value=\""+k.kategorije.get(request.getParameter("ime")).getGpu_jezgra()+"\"/>");
				if(!gpuBul)
					out.println("<p>RAM nije validno</p>");
				out.println("		<br>");
